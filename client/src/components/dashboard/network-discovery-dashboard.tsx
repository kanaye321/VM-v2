...
import { Server, Activity, Search, Database, Eye, Laptop, Monitor, Cpu, HardDrive, Usb, Package } from "lucide-react";
...
export default function NetworkDiscoveryDashboard({ hosts, onViewDetails }: NetworkDiscoveryDashboardProps) {
  ...
  const onlineDevices = hosts.filter(h => h.status === 'online').length;
  const offlineDevices = hosts.filter(h => h.status === 'offline').length;
  const newDevices = hosts.filter(h => h.status === 'new').length;
  const importedDevices = hosts.filter(h => h.status === 'imported').length;

  // Group devices by operating system
  const osByCount = hosts.reduce((acc, host) => {
  ...
  const manufacturerCount = hosts.reduce((acc, host) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">No devices discovered</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">New Discoveries</CardTitle>
              <Search className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">0</div>
              <p className="text-xs text-muted-foreground">Recently found</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          ...
          <CardContent className="h-[400px] flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">Start a network scan to discover devices on your network</p>
            </div>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">{importedDevices}</div>
            <p className="text-xs text-muted-foreground">Added to inventory</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Breakdown by detected OS</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {Object.entries(osByCount).map(([os, count]) => (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Hardware vendors detected</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Detailed list of all discovered network devices</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <TableBody>
                {hosts.map((host) => (
                  <TableRow key={host.id}>
                    <TableCell>
                      {host.status === 'online' && <Badge className="bg-green-500">Online</Badge>}
                      {host.status === 'offline' && <Badge variant="outline">Offline</Badge>}
                      {host.status === 'new' && <Badge className="bg-blue-500">New</Badge>}
                      {host.status === 'imported' && <Badge className="bg-purple-500">Imported</Badge>}
                    </TableCell>
                    <TableCell className="font-medium">
                      <div>
                        <div>{host.hostname || 'Unknown'}</div>
                        {host.systemInfo?.computerName && host.systemInfo.computerName !== host.hostname && (
                          <div className="text-xs text-muted-foreground">{host.systemInfo.computerName}</div>
                        )}
                      </div>
                    </TableCell>
                    <TableCell>{host.ipAddress}</TableCell>
                    <TableCell className="font-mono text-sm">{host.macAddress || 'Unknown'}</TableCell>
                    <TableCell>
                      <div>
                        <div>{host.systemInfo?.os || 'Unknown'}</div>
                        {host.systemInfo?.osVersion && (
                          <div className="text-xs text-muted-foreground">{host.systemInfo.osVersion}</div>
                        )}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      {host.lastSeen ? new Date(host.lastSeen).toLocaleDateString() : 'Unknown'}
                    </TableCell>
                    <TableCell>
