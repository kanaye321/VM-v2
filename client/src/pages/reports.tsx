...
export default function Reports() {
  ...
  const getReportData = (reportType: string) => {
    ...
    try {
      ...
      switch (reportType) {
        case 'vm-inventory':
          // Get VM inventory data with approval history
          let vmData = [];
          try {
            const vmResponse = await fetch('/api/vm-inventory');
            if (vmResponse.ok) {
              vmData = await vmResponse.json();
              
              // Fetch approval history for each VM
              for (let vm of vmData) {
                try {
                  const historyResponse = await fetch(`/api/vm-inventory/${vm.id}/approval-history`);
                  if (historyResponse.ok) {
                    const history = await historyResponse.json();
                    vm.approvalHistory = history;
                    vm.approvalHistoryCount = history.length;
                    vm.lastApprovalChange = history.length > 0 ? history[0].changedAt : null;
                  }
                } catch (error) {
                  console.error(`Error fetching approval history for VM ${vm.id}:`, error);
                  vm.approvalHistory = [];
                  vm.approvalHistoryCount = 0;
                  vm.lastApprovalChange = null;
                }
              }
            }
          } catch (error) {
          ...
          result = vmData.map(vm => ({
            'VM ID': vm.vmId || 'N/A',
            'VM Name': vm.vmName || 'N/A',
            'Status': vm.vmStatus || 'N/A',
            'VM IP': vm.vmIp || 'N/A',
            'OS': vm.vmOs || 'N/A',
            'CPU Count': vm.cpuCount || 'N/A',
            'Memory (GB)': vm.memoryGb || 'N/A',
            'Disk Capacity (GB)': vm.diskCapacityGb || 'N/A',
