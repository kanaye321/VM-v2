...
export default function UserManagement() {
  ...
  const { data: apiRoles = [], isLoading: rolesLoading } = useQuery<Role[]>({
    ...
    refetchInterval: 5000, // Refresh every 5 seconds to keep counts updated
  });
  ...
  const addUserMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/roles"] }); // Refresh role counts
  ...
  const assignUserToRoleMutation = useMutation({
    mutationFn: async (data: { userId: number; roleId: number }) => {
      const response = await apiRequest("PATCH", `/api/users/${data.userId}`, { 
        roleId: data.roleId,
        isAdmin: false // Remove admin status when assigning role
      });
      if (!response.ok) {
        const error = await response.json();
  ...
  // Combine API roles with custom roles, giving priority to API roles
          ...
          <Card>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {allRoles.map((role) => {
                    // Calculate real-time user count
                    const actualUserCount = users.filter(user => {
                      if (role.id === 1) { // Administrator role
                    ...
                    return (
                      <TableRow key={role.id}>
                        ...
                        <TableCell>{actualUserCount}</TableCell>
                        <TableCell>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  {users
                    .filter(user => {
                      // For Administrator role, show non-admin users
                      if (selectedRoleForAssignment?.id === 1) {
                        return !user.isAdmin;
                      }
                      // For other roles, show users who don't have this role and aren't admin
